...
function RippleButton({ children, className = "", href, onClick }) {
  ...
  return (
    <Tag href={href} className={`relative overflow-hidden ${className}`} onClick={handleClick}>
  ...
  return (
    <footer style={{ background: "#0a1628" }}>
      ...
      <div className="border-t border-white/5 py-6">
        <div className="max-w-7xl mx-auto px-6 text-xs text-slate-600">
          <span>© 2026 MKL Enterprises. All rights reserved. · NAD Junction, Visakhapatnam AP 530027</span>
        </div>
      </div>
    </footer>
  );
...
const AMC_PLANS = [
  {
    ...
    features: ["2 scheduled services", "Filter check & clean", "All brand models", "Phone support", "Genuine parts used"],
    popular: false,
  },
  {
    name: "Standard AMC",
    ...
    features: ["4 scheduled services", "All filter replacements", "Priority dispatch", "1 free repair visit", "Digital service reports", "WhatsApp support"],
    popular: true,
  },
  {
    name: "Premium AMC",
    ...
    features: ["6 scheduled services", "All filters + membrane", "Unlimited repair visits", "24/7 emergency support", "Spare parts covered", "Free water quality tests"],
    popular: false,
  },
];

const COMPARISON_ROWS = [
  { feature: "Monthly Cost", rental: "₹399–₹599/mo", owning: "₹0 (post-purchase)" },
  { feature: "Upfront Cost", rental: "₹0", owning: "₹8,000–₹25,000" },
  { feature: "Installation", rental: "Free", owning: "₹500–₹1,500" },
  { feature: "Filter Changes (annual)", rental: "Included", owning: "₹2,000–₹5,000" },
  { feature: "Repairs", rental: "Included", owning: "₹500–₹5,000 per call" },
  { feature: "Relocation", rental: "Free", owning: "₹500–₹1,000" },
  { feature: "Model Upgrade", rental: "Anytime, free", owning: "Buy new purifier" },
  { feature: "Annual Total (approx)", rental: "₹4,788–₹7,188", owning: "₹12,000–₹35,000 (yr 1)" },
];
...
export default function PlansPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(".plans-hero-badge", { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, ease: "power3.out", delay: 0.5 });
      gsap.fromTo(".plans-hero-title", { y: 40, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, ease: "power3.out", delay: 0.7 });
      gsap.fromTo(".plans-hero-sub", { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, ease: "power3.out", delay: 0.9 });
      gsap.to(".plans-blob-1", { y: -20, duration: 6, repeat: -1, yoyo: true, ease: "sine.inOut" });

      gsap.fromTo(".rental-card", { y: 80, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, stagger: 0.15, ease: "back.out(1.2)", scrollTrigger: { trigger: rentalRef.current, start: "top 75%", once: true } });
      gsap.fromTo(".amc-card", { y: 60, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, stagger: 0.15, ease: "back.out(1.2)", scrollTrigger: { trigger: amcRef.current, start: "top 75%", once: true } });
      gsap.fromTo(".compare-row", { x: -30, opacity: 0 }, { x: 0, opacity: 1, duration: 0.5, stagger: 0.06, ease: "power3.out", scrollTrigger: { trigger: compareRef.current, start: "top 80%", once: true } });
    }, document.body);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <>
      ...
      <section ref={heroRef} className="relative pt-36 pb-20 overflow-hidden" style={{ background: "linear-gradient(135deg,#0a1628 0%,#1e3a8a 55%,#1e4db7 100%)" }}>
        <div className="plans-blob-1 absolute -top-24 -right-24 opacity-20 pointer-events-none"><WaterBlob color="#60a5fa" opacity={1} size={450} /></div>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
          <div className="plans-hero-badge inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-blue-200 text-sm font-semibold tracking-wider uppercase">Transparent Pricing</span>
          </div>
          <h1 className="plans-hero-title text-5xl md:text-6xl font-black text-white leading-tight mb-6">
            Simple, Honest<br />
            <span className="text-transparent bg-clip-text" style={{ backgroundImage: "linear-gradient(135deg,#60a5fa,#93c5fd,#bfdbfe)" }}>Plans & Pricing</span>
          </h1>
          <p className="plans-hero-sub text-lg text-blue-200 leading-relaxed">No hidden fees. No surprises. Just clean water at a fixed monthly cost — with everything included.</p>
        </div>
      </section>
      ...
      <section className="bg-[#f1f5f9] pt-16 pb-4">
        <div className="max-w-xs mx-auto px-6">
          <div className="flex bg-white rounded-2xl p-1.5 border border-blue-100 shadow-sm">
            <button onClick={() => setActiveTab("rental")} className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-all ${activeTab === "rental" ? "bg-[#1e3a8a] text-white shadow-lg shadow-blue-900/30" : "text-slate-500 hover:text-[#1e3a8a]"}`}>Rental Plans</button>
            <button onClick={() => setActiveTab("amc")} className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-all ${activeTab === "amc" ? "bg-[#1e3a8a] text-white shadow-lg shadow-blue-900/30" : "text-slate-500 hover:text-[#1e3a8a]"}`}>AMC Plans</button>
          </div>
        </div>
      </section>
        ...
        <section ref={rentalRef} className="pb-24 bg-[#f1f5f9]">
          <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-12 pt-12">
              <h2 className="text-4xl font-black text-slate-900 mb-3">Water Purifier <span className="text-[#1e3a8a]">Rental Plans</span></h2>
              <p className="text-slate-500 max-w-lg mx-auto">All plans include free installation, all maintenance, filter replacements, and repairs. Choose longer for bigger savings.</p>
            </div>
            <div className="grid md:grid-cols-3 gap-6 items-start">
              {RENTAL_PLANS.map((plan, i) => (
                <div key={i} className={`rental-card relative rounded-3xl overflow-hidden transition-all duration-300 hover:-translate-y-3 ${plan.popular ? "ring-2 ring-blue-300 ring-offset-4 ring-offset-[#f1f5f9] md:-mt-4" : ""}`}>
                  {plan.popular && <div className="absolute top-4 right-4 bg-blue-300 text-[#1e3a8a] text-xs font-black px-3 py-1 rounded-full z-10">⭐ Most Popular</div>}
                  {plan.saving && <div className="absolute top-4 left-4 bg-green-400 text-green-900 text-xs font-black px-3 py-1 rounded-full z-10">{plan.saving}</div>}
                  ...
                  <div className={`bg-gradient-to-br ${plan.color} p-8`}>
                    ...
                    <div className="text-blue-300/70 text-xs font-medium mb-6">{plan.total}</div>
                    <RippleButton href="/contact" className="w-full py-3.5 bg-white text-[#1e3a8a] font-bold rounded-2xl hover:bg-blue-50 transition-colors text-sm text-center block shadow-lg shadow-black/10">
                      Get Started →
                    </RippleButton>
                  </div>
                  ...
                  <div className="bg-white p-8">
                    <ul className="space-y-3">
                      {plan.features.map((f, j) => (
                        <li key={j} className={`flex items-center gap-3 text-sm ${f.included ? "text-slate-700" : "text-slate-300"}`}>
                          <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs flex-shrink-0 font-bold ${f.included ? "bg-[#1e3a8a] text-white" : "bg-slate-100 text-slate-300"}`}>{f.included ? "✓" : "✕"}</span>
                          {f.text}
                        </li>
                      ))}
                    </ul>
            ...
            <p className="text-center text-slate-400 text-sm mt-8">All plans · Free doorstep delivery · Genuine spare parts · Certified technicians · Servicing Vizag since 2018</p>
          </div>
        </section>
      )}
      ...
      {activeTab === "amc" && (
        <section ref={amcRef} className="pb-24 bg-[#f1f5f9]">
          <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-12 pt-12">
              <h2 className="text-4xl font-black text-slate-900 mb-3">Annual Maintenance <span className="text-[#1e3a8a]">Contract Plans</span></h2>
              <p className="text-slate-500 max-w-lg mx-auto">Own a purifier? Keep it running perfectly with our AMC plans. Fixed yearly cost, zero surprise bills, priority service.</p>
            </div>
            <div className="grid md:grid-cols-3 gap-6 items-start">
              {AMC_PLANS.map((plan, i) => (
                <div key={i} className={`amc-card relative rounded-3xl overflow-hidden transition-all duration-300 hover:-translate-y-2 ${plan.popular ? "ring-2 ring-blue-300 ring-offset-4 ring-offset-[#f1f5f9]" : ""}`}>
                  {plan.popular && <div className="absolute top-4 right-4 bg-blue-300 text-[#1e3a8a] text-xs font-black px-3 py-1 rounded-full z-10">Most Popular</div>}
                  <div className={`p-8 ${plan.popular ? "bg-[#1e3a8a]" : "bg-white border border-slate-100"}`}>
                    <h3 className={`font-black text-xl mb-1 ${plan.popular ? "text-white" : "text-slate-900"}`}>{plan.name}</h3>
                    <p className={`text-sm mb-4 ${plan.popular ? "text-blue-200" : "text-slate-400"}`}>{plan.desc}</p>
                    <div className="flex items-baseline gap-1 mb-6">
                      <span className={`text-4xl font-black ${plan.popular ? "text-white" : "text-[#1e3a8a]"}`}>{plan.price}</span>
                      <span className={`text-sm ${plan.popular ? "text-blue-200" : "text-slate-400"}`}>{plan.period}</span>
                    </div>
                    <ul className="space-y-3 mb-8">
                      {plan.features.map((f, j) => (
                        <li key={j} className={`flex items-center gap-3 text-sm ${plan.popular ? "text-blue-100" : "text-slate-600"}`}>
                          <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs flex-shrink-0 ${plan.popular ? "bg-white/20 text-white" : "bg-blue-100 text-[#1e3a8a]"}`}>✓</span>
                          {f}
                        </li>
                      ))}
                    </ul>
                    <RippleButton href="/contact" className={`w-full py-3.5 font-bold rounded-2xl text-sm text-center block transition-colors ${plan.popular ? "bg-white text-[#1e3a8a] hover:bg-blue-50 shadow-lg" : "bg-[#1e3a8a] text-white hover:bg-blue-800"}`}>
                      Get AMC →
                    </RippleButton>
                  </div>
                </div>
      ...
      <section ref={compareRef} className="py-24 bg-white">
        <div className="max-w-4xl mx-auto px-6">
          <div className="text-center mb-12">
            <span className="inline-block bg-blue-100 text-[#1e3a8a] text-xs font-bold tracking-widest uppercase px-4 py-2 rounded-full mb-4">Cost Analysis</span>
            <h2 className="text-4xl font-black text-slate-900">Renting vs <span className="text-[#1e3a8a]">Buying</span></h2>
            <p className="text-slate-500 mt-3">See the real numbers — renting is almost always smarter in the first 2 years.</p>
          </div>
          <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
            <div className="grid grid-cols-3 bg-[#1e3a8a] text-white text-sm font-bold">
              <div className="p-4">Feature</div>
              <div className="p-4 text-center border-l border-white/10 text-blue-300">🏠 Renting (MKL)</div>
              <div className="p-4 text-center border-l border-white/10">💰 Buying (own)</div>
            </div>
            {COMPARISON_ROWS.map((row, i) => (
              <div key={i} className={`compare-row grid grid-cols-3 text-sm border-b border-slate-100 last:border-0 ${i % 2 === 0 ? "bg-slate-50/50" : "bg-white"}`}>
                <div className="p-4 font-semibold text-slate-700">{row.feature}</div>
                <div className="p-4 text-center border-l border-slate-100">
                  <span className="inline-block bg-blue-100 text-[#1e3a8a] font-bold px-3 py-1 rounded-lg text-xs">{row.rental}</span>
                </div>
                <div className="p-4 text-center border-l border-slate-100 text-slate-500 font-medium">{row.owning}</div>
              </div>
            ))}
          </div>
          <p className="text-slate-400 text-xs text-center mt-4">* Owning costs are estimates based on average market prices. Actual costs may vary by brand and model.</p>
        </div>
      </section>
      ...
      <section className="py-20 relative overflow-hidden" style={{ background: "linear-gradient(135deg,#0a1628 0%,#1e3a8a 100%)" }}>
        ...
        <div className="max-w-3xl mx-auto px-6 text-center relative z-10">
          <h2 className="text-4xl font-black text-white mb-4">Still Deciding?</h2>
          <p className="text-blue-200 mb-8 text-lg">WhatsApp us and we'll help you choose the best plan for your needs — no pressure, no sales pitch.</p>
          <div className="flex flex-wrap gap-4 justify-center">
            <a href="https://wa.me/918179019929?text=Hi!%20I%20want%20to%20know%20more%20about%20your%20rental%20plans." className="flex items-center gap-2 bg-green-500 text-white font-bold px-8 py-4 rounded-2xl hover:bg-green-600 transition-colors shadow-xl">💬 WhatsApp to Discuss</a>
            <a href="tel:8179019929" className="flex items-center gap-2 bg-white text-[#1e3a8a] font-bold px-8 py-4 rounded-2xl hover:bg-blue-50 transition-colors shadow-xl shadow-black/20">📞 Call Now</a>
          </div>
        </div>
      </section>
